...
void EtherCheck::push(int, Packet* p){
	...
	if (ntohs(eth_h->ether_type) == 0x0800){
		// ICMP message
		if (ip_h->ip_p == 1){
			...
			if (icmp_h->icmp_type == 0 && !_monitorMN->isHome()){
				...
				for (int i=0; i<6; i++) newEtherHeader->ether_dhost[i] = etherDest[i];
				for (int i=0; i<6; i++) newEtherHeader->ether_shost[i] = eth_h->ether_shost[i];
